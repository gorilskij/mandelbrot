// Tile colouring: a tile's iteration counts → one mip level of its colour
// texture. Runs whenever a tile's contents or the palette change, so a
...
@group(1) @binding(0) var iters: texture_2d<u32>;
@group(1) @binding(1) var out: texture_storage_2d<rgba8unorm, write>;
...
fn add_known(p: vec2<i32>, n: i32, sum: ptr<function, vec3<f32>>, k: ptr<function, f32>) {
    if (any(p < vec2<i32>(0)) || any(p >= vec2<i32>(n))) { return; }
    let v = textureLoad(iters, p, 0).r;
    if ((v & GAP) != 0u) { return; }
    *sum += colour(v);
    *k += 1.0;
}
...
fn texel(p: vec2<i32>, n: i32) -> vec3<f32> {
    let v = textureLoad(iters, p, 0).r;
    if ((v & GAP) == 0u) { return colour(v); }
    var sum = vec3<f32>(0.0);
    var k = 0.0;
    add_known(p + vec2<i32>(-1, 0), n, &sum, &k);
    add_known(p + vec2<i32>(1, 0), n, &sum, &k);
    add_known(p + vec2<i32>(0, -1), n, &sum, &k);
    add_known(p + vec2<i32>(0, 1), n, &sum, &k);
...
fn main(@builtin(global_invocation_id) id: vec3<u32>) {
    ...
    for (var y = 0; y < f; y++) {
        for (var x = 0; x < f; x++) {
            sum += texel(origin + vec2<i32>(x, y), n);
        }
    }
    let lin = sum / f32(f * f);
    textureStore(out, vec2<i32>(id.xy), vec4<f32>(linear_to_srgb(lin), 1.0));
}
